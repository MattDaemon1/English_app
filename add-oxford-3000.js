...
console.log(`   Mots a traduire: ${untranslatedWords} (${Math.round(untranslatedWords / totalWords * 100)}%)`); ('📚 Extraction et ajout des 3000 mots Oxford...\n');
...
console.log(`🔍 Trouvé ${extractedWords.length} mots à traiter`);
...
function addWordsToDatabase() {
    ...
    console.log('\n🚀 Ajout des mots en base de données...');
...
console.log(`   ➕ Nouveaux mots ajoutés: ${result.added}`);
console.log(`   ⏩ Mots existants ignorés: ${result.skipped}`);
console.log(`   🔄 Mots mis à jour: ${result.updated}`);
console.log(`\n📚 Statistiques de la base:');
console.log(`   📖 Total des mots: ${ totalWords }`);
console.log(`   ✅ Mots traduits: ${ translatedWords }(${ Math.round(translatedWords / totalWords * 100) } %)`);
console.log(`   ⚠️  Mots à traduire: ${ untranslatedWords }(${ Math.round(untranslatedWords / totalWords * 100) } %)`);

console.log('\n🎉 Base de données Oxford 3000 mise à jour avec succès !');
...
if (untranslatedWords > 0) {
    ...
    console.log('   3. Intégrer un API de traduction pour automatiser le processus');
}
